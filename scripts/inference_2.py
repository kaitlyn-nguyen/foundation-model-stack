...
class ForwardModule(torch.nn.Module):
    ...
    def forward(self, input_ids, past_key_value_states):
...
if args.export_model:
    logger.info("Exporting the model's forward call with cache...")

    try:
        # Example initial input
        example_input_ids = ids

        # Create dummy past_key_value_states
        past_key_value_states = [
            (torch.zeros((1, 4, 194, 48), dtype=torch.float16, device=device), 
             torch.zeros((1, 4, 194, 48), dtype=torch.float16, device=device))
            for _ in range(5)
        ]

        # Example inputs including past_key_value_states
        example_inputs = (example_input_ids, past_key_value_states)
        
        # Create forward module instance
        forward_module = ForwardModule(model)
        
        # Export the forward call using the wrapper module
        exported_program = export(forward_module, args=example_inputs)
        logger.info("Model's forward call with cache exported successfully")

        save(exported_program, args.export_path)
        logger.info("Model's forward call with cache saved successfully")

        # Load the exported forward call
        loaded_program = load(args.export_path).module()
        logger.info("Loaded the exported model's forward call with cache successfully")

        # Run forward pass with the same inputs on the loaded model
        with torch.no_grad():
